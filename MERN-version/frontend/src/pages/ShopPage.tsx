...
const ShopPage: React.FC = () => {
  ...
  const purchaseMutation = useMutation({
    mutationFn: (orderData: { items: Array<{ productId: string; quantity: number }> }) =>
      apiClient.checkout({ 
  ...
  const getCartTotal = () => {
    return cart.reduce((total, item) => {
      const product = products?.data?.find((p: any) => p._id === item.productId);
  ...
  const categories = Array.from(new Set(products?.data?.map((p: any) => p.category) || []));
