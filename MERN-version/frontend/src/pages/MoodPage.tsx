...
const MoodPage: React.FC = () => {
  ...
  const [selectedMood, setSelectedMood] = useState<'excellent' | 'good' | 'okay' | 'not-great' | 'poor' | ''>('');
  ...
  const submitMoodMutation = useMutation({
    mutationFn: (moodData: { mood: string; comment?: string }) =>
      apiClient.submitMood({ mood: moodData.mood as any, comment: moodData.comment }),
    onSuccess: () => {
  ...
  const getMoodStats = () => {
    ...
    const moodCounts = last7Days.reduce((acc: any, mood) => {
      acc[mood.mood] = (acc[mood.mood] || 0) + 1;
      return acc;
    }, {});
    
    const mostCommon = Object.entries(moodCounts).sort(([,a]: any, [,b]: any) => b - a)[0];
