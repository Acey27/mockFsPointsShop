...
async function seedDatabase() {
  try {
    ...
    const regularUsers = createdUsers.filter((user: any) => user.role === 'user');
    ...
    const moods = ['excellent', 'good', 'okay', 'not-great', 'poor'] as const;
    const moodComments = [
      'Great day today!',
    ...
    for (let i = 0; i < 15; i++) {
      const randomUser = regularUsers[Math.floor(Math.random() * regularUsers.length)];
      const randomMood = moods[Math.floor(Math.random() * moods.length)] as 'excellent' | 'good' | 'okay' | 'not-great' | 'poor';
