...
router.get('/transactions', requireAuth, async (req, res) => {
  try {
    ...
    // Build query to find ONLY cheer-related transactions (heartbits given/received)
    // Exclude shop purchases and other non-cheer transactions
    const baseQuery= {
      $or: [
        { fromUserId: req.user._id },
        { toUserId: req.user._id },
      ...
      // Only include cheer-related transactions
      $and: [
        {
          $or: [
            { 'metadata.type': 'cheer' },
            { 'metadata.transactionType': 'cheer' },
            { description: { $regex: /cheer|heartbit/i } }
          ]
        }
      ]
    };
